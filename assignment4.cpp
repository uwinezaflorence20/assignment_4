...
//enumeration that hold possible genres
enum Genre {
    Fiction,
    NonFiction,
    Periodical,
...
//we are going to define the class Patron
class Patron {
    ...
    public:
        //constructor
        Patron(string u, string c, int f) : userNname(u), cardNumber(c), owedFees(f) {}
...
class Library {
    ...
    public:
        ...
        //Find_debtors() finds debtor patrons in the patrons vector
        vector<Patron> Find_debtors() {
            for(auto i : patrons) {
                if(i.getOwedFees() > 0) {
                    debtors.push_back(i);
                }
        ...
        void checkOut(Patron& patron, Book& book, string activity, string date) {
           Find_debtors();// find debtors
            ...
            if(patronIter == patrons.end() || bookIter == books.end()) {
                ...
                cout<<"Activity that have been recorded"<<endl;
            }
        }
...
int main() {
    ...
    if (book1 == book2) {
        cout << "ISBMs of Book 1 and Book 2 are the same." <<endl;
    } else {
        cout << "ISBMs of Book 1 and Book 2 are different." <<endl;
    }
